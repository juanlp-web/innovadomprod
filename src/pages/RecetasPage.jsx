...
export function RecetasPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    preparationTime: "",
    cookingTime: "",
    servings: "", // Se llenará automáticamente con batchInfo.quantity
  ...
  const categories = ["bebida", "alimento", "postre", "otro"];
  ...
  const generateBatchNumber = () => {
    ...
    return `LOTE-${year}${month}${day}-${hours}${minutes}${seconds}`;
  };
  ...
  const openProductModal = (index) => {
    ...
    // Determinar el tipo de producto según el contexto
    const productType = index === "productToProduce" 
      ? "producto_terminado" 
      : "materia_prima";
  ...
  const handleProductSearch = (searchTerm) => {
    ...
    // Determinar el tipo de producto según el contexto
    const productType = selectedIngredientIndex === "productToProduce" 
      ? "producto_terminado" 
      : "materia_prima";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.productToProduce) {
        showError(
          "Debe seleccionar el producto que se produce con esta receta."
        );
        return;
      }
      ...
      if (
        !formData.batchInfo.expirationDate ||
        !formData.batchInfo.quantity ||
        String(formData.batchInfo.quantity).trim() === "" ||
        !formData.batchInfo.unit ||
        String(formData.batchInfo.unit).trim() === ""
      ) {
        showError(
          "Debe completar la información del lote: fecha de vencimiento, cantidad y unidad."
        );
        return;
      }
      ...
      const validIngredients = formData.ingredients.filter(
        (ing) => ing.product || (ing.name && ing.quantity)
      );
      if (validIngredients.length === 0) {
        showError(
          "Debe agregar al menos un ingrediente con producto del inventario o ingrediente genérico."
        );
        return;
      }

      // Generar número de lote automático si no se especificó uno manual
      const finalBatchNumber = (formData.batchInfo.batchNumber && String(formData.batchInfo.batchNumber).trim()) || generateBatchNumber();
      ...
      const recipeData = {
        ...
        batchInfo: {
          ...formData.batchInfo,
          batchNumber: finalBatchNumber, // Usar el lote manual o el generado automáticamente
      ...
      setFormData({
        ...
        batchInfo: {
          batchNumber: "", // Se generará automáticamente si se deja vacío
          productionDate: new Date(),
  ...
  const handleEdit = (recipe) => {
    ...
    const mapRecipeData = () => {
      ...
      if (recipe.productToProduce) {
        if (
          typeof recipe.productToProduce === "object" &&
          recipe.productToProduce._id
        ) {
          // Producto populado
          productToProduce = products.find(
            (p) => p._id === recipe.productToProduce._id
          ) || {
            _id: recipe.productToProduce._id,
            name: recipe.productToProduce.name || "Producto no encontrado",
          };
        } else {
          // Solo ID del producto
          productToProduce = products.find(
            (p) => p._id === recipe.productToProduce
      ...
      setFormData({
        ...
        servings: recipe.servings || recipe.batchInfo?.quantity || "",
        productToProduce: productToProduce, // Producto completo
        batchInfo: {
          batchNumber: recipe.batchInfo?.batchNumber || "",
          productionDate: recipe.batchInfo?.productionDate ? new Date(recipe.batchInfo.productionDate) : new Date(),
          expirationDate: recipe.batchInfo?.expirationDate ? new Date(recipe.batchInfo.expirationDate) : new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
          quantity: recipe.batchInfo?.quantity || "",
          unit: recipe.batchInfo?.unit || "unidad",
        },
        ingredients:
            ...
            if (ing.product) {
              ...
              if (typeof ing.product === "object" && ing.product._id) {
                ...
                product = products.find((p) => p._id === ing.product._id) || {
                  _id: ing.product._id,
                  name: ing.product.name || "Producto no encontrado",
                };
              } else {
                // Solo ID del producto
  ...
  const handleStatusChange = async (recipeId, newStatus, quantity = 1) => {
    try {
      ...
      const actualQuantity = recipe?.batchInfo?.quantity || 1; // Usar la cantidad del lote
      ...
      if (newStatus === "completada") {
        ...
        const ingredientsWithProducts =
          recipe.ingredients?.filter((ing) => ing.product) || [];
      ...
      if (result && result.stockChange) {
        const {
          productName: changedProductName,
          change,
          newStock,
        } = result.stockChange;
        ...
        if (
          ...
          result.ingredientsConsumed.length > 0
        ) {
        ...
        if (
          ...
          result.ingredientsRestored.length > 0
        ) {
      ...
      if (newStatus === "completada") {
        info(
          `Stock del producto aumentado e ingredientes consumidos exitosamente`
        );
      } else if (currentStatus === "completada") {
        warning(
          `Stock del producto disminuido e ingredientes restaurados exitosamente`
        );
      }
    } catch (error) {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    const matchesCategory =
      filterCategory === "todos" || recipe.category === filterCategory;
    const matchesDifficulty =
      filterDifficulty === "todos" || recipe.difficulty === filterDifficulty;
    const matchesStatus =
      filterStatus === "todos" || recipe.status === filterStatus;
    return (
  ...
  useEffect(() => {
    if (showProductModal && selectedIngredientIndex) {
      // Determinar el tipo de producto según el contexto
      const productType = selectedIngredientIndex === "productToProduce" 
        ? "producto_terminado" 
        : "materia_prima";
  ...
  }, [showProductModal, selectedIngredientIndex, fetchProducts]);
  ...
  // Sincronizar servings con batchInfo.quantity
  useEffect(() => {
    if (formData.batchInfo.quantity) {
      setFormData(prev => ({
        ...prev,
        servings: formData.batchInfo.quantity
      }));
    }
  }, [formData.batchInfo.quantity]);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="flex-1">
          ...
          <p className="text-gray-600 text-sm sm:text-base">
            Administra formulaciones y recetas de productos
          </p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-3 text-xs sm:text-sm">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-600">
                En Preparación:{" "}
                {recipes.filter((r) => r.status === "en_preparacion").length}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-600">
                Completadas:{" "}
                {recipes.filter((r) => r.status === "completada").length}
              </span>
            </div>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-gray-600">
                Descartadas:{" "}
                {recipes.filter((r) => r.status === "descartada").length}
              </span>
            </div>
          </div>
          ...
          <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-800">
            <div className="flex items-start space-x-2">
              ...
              <span className="text-xs leading-relaxed">
                <strong>Gestión de Stock:</strong> Las recetas "Completadas"
                aumentan el stock del producto producido y consumen ingredientes
                del inventario. Cambiar su estado desde "Completada" lo revierte
                automáticamente.
              </span>
            </div>
          </div>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="text-sm text-blue-800">
                  ...
                  <ul className="space-y-1 text-xs">
                    <li>
                      • <strong>En Preparación:</strong> Estado por defecto al
                      crear una receta
                    </li>
                    <li>
                      • <strong>Completada:</strong> Aumenta el stock del
                      producto producido y consume ingredientes del inventario
                    </li>
                    <li>
                      • <strong>Descartada:</strong> Marca la receta como no
                      utilizada
                    </li>
                    <li>
                      • <strong>Duplicar:</strong> Crea una copia de la receta
                      con estado "En Preparación"
                    </li>
                    <li>
                      • <strong>⚠️ Importante:</strong> Cambiar desde
                      "Completada" a otro estado disminuye el stock del producto
                      y restaura los ingredientes
                    </li>
                  </ul>
                </div>
              </div>
                    ...
                    {formData.productToProduce ? (
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-gray-900">
                          {formData.productToProduce.name}
                        </span>
                        <span className="text-gray-500 text-sm">
                          ({formData.productToProduce.sku})
                        </span>
                      </div>
                    ) : (
            ...
            <div className="p-4 bg-purple-50 border border-purple-200 rounded-lg mb-4">
              <div className="flex items-start space-x-3">
                ...
                <div className="text-sm text-purple-800">
                  <p className="font-medium mb-1">
                    Información sobre Números de Lote:
                  </p>
                  <ul className="space-y-1 text-xs">
                    <li>
                      • <strong>Manual:</strong> Puede especificar un número de lote personalizado
                    </li>
                    <li>
                      • <strong>Automático:</strong> Si deja el campo vacío, se generará automáticamente con formato LOTE-YYYYMMDD-HHMMSS
                    </li>
                    <li>
                      • <strong>Botón "Auto":</strong> Genera un número de lote inmediatamente
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    type="button"
                    onClick={() => handleBatchInfoChange("batchNumber", generateBatchNumber())}
                    variant="outline"
                    size="sm"
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Si se deja vacío, se generará automáticamente al guardar
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleBatchInfoChange("quantity", e.target.value)
                  }
                  required
                  min="1"
                  className="w-full input-field"
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    handleBatchInfoChange("unit", e.target.value)
                  }
                  className="w-full input-field"
                  required
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  type="date"
                  value={
                    formData.batchInfo.productionDate
                      ? new Date(formData.batchInfo.productionDate)
                          .toISOString()
                          .split("T")[0]
                      : ""
                  }
                  ...
                  className="w-full input-field"
                />
                <p className="text-xs text-gray-500 mt-1">Por defecto: Hoy</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <input
                  ...
                  value={
                    formData.batchInfo.expirationDate
                      ? new Date(formData.batchInfo.expirationDate)
                          .toISOString()
                          .split("T")[0]
                      : ""
                  }
                ...
                {formData.ingredients.map((ingredient, index) => (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {ingredient.product && (
                              <Button
                                type="button"
                                onClick={() =>
                                  updateIngredient(index, "product", null)
                                }
                                variant="ghost"
                                size="sm"
                      ...
                      <div
                        className={ingredient.isGeneric ? "w-full" : "flex-1"}
                      >
                        <label className="block text-xs font-medium text-gray-600 mb-1">
                          {ingredient.isGeneric
                            ? "Nombre del Ingrediente Genérico *"
                        ...
                        <input
                          ...
                          title={
                            ingredient.product
                              ...
                              : ingredient.isGeneric
                              ? "Costo manual para ingrediente genérico"
                              : "Costo manual"
                          }
                        />
                      </div>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {!ingredient.product && !ingredient.isGeneric && (
                            <Button
                              type="button"
                              onClick={() =>
                                updateIngredient(index, "isGeneric", true)
                              }
                              variant="ghost"
                              size="sm"
                          ...
                          {ingredient.isGeneric && (
                            <Button
                              type="button"
                              onClick={() =>
                                updateIngredient(index, "isGeneric", false)
                              }
                              variant="ghost"
                              size="sm"
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-xs text-blue-600">
                      <div className="text-center">
                        <div>
                          {
                            formData.ingredients.filter((ing) => ing.product)
                              .length
                          }{" "}
                          de {formData.ingredients.length} ingredientes
                          vinculados al inventario
                        </div>
                        <div className="text-green-600">
                          {formData.ingredients.filter((ing) => ing.product)
                            ...
                            `${
                              formData.ingredients.filter((ing) => ing.product)
                                .length
                            } con costos automáticos`}
                        </div>
                        <div className="text-blue-600">
                          {formData.ingredients.filter((ing) => ing.isGeneric)
                            .length > 0 &&
                            `${
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
                      <div
                        className={`px-3 py-1 rounded-full text-xs font-medium ${statusInfo.bgColor} ${statusInfo.color} border ${statusInfo.borderColor} self-start`}
                      >
                        {statusInfo.label}
                      </div>
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      {recipe.batchInfo?.batchNumber && (
                        <div className="flex items-center justify-between text-sm">
                          <span className="text-gray-600">Lote:</span>
                          <span className="font-medium text-purple-600 font-mono text-xs">
                            {recipe.batchInfo.batchNumber}
                          </span>
                        </div>
                      )}
                    ...
                    <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                        ...
                        {(() => {
                          ...
                          return (
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${statusInfo.bgColor} ${statusInfo.color} border ${statusInfo.borderColor} self-start`}
                            >
                              {statusInfo.label}
                            </span>
                          );
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                          ...
                          <Button
                            ...
                            title={
                              recipe.status === "completada"
                                ? `Volver a En Preparación - Disminuirá el stock de "${
                                    recipe.productToProduce?.name
                                  }" en ${recipe.batchInfo?.quantity || 1} unidad(es)`
                                : "Volver a En Preparación"
                            }
                          >
                        ...
                        {recipe.status !== "completada" && (
                          <Button
                            ...
                            title={`Completar - Aumentará el stock de "${
                              recipe.productToProduce?.name
                            }" en ${recipe.batchInfo?.quantity || 1} unidad(es)`}
                          >
                            <Target className="w-3 h-3 mr-1" />
                            <span className="hidden sm:inline">
                              Completar (+{recipe.batchInfo?.quantity || 1})
                            </span>
                        ...
                        {recipe.status !== "descartada" && (
                          <Button
                            ...
                            title={
                              recipe.status === "completada"
                                ? `Descartar - Disminuirá el stock de "${
                                    recipe.productToProduce?.name
                                  }" en ${recipe.batchInfo?.quantity || 1} unidad(es)`
                                : "Descartar receta"
                            }
                      ...
                      {recipe.status === "completada" && (
                        <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded text-xs text-green-700">
                          <div className="flex items-center space-x-2">
                            ...
                            <span>
                              ✓ Stock del producto{" "}
                              <strong>{recipe.productToProduce?.name}</strong>{" "}
                              aumentado en{" "}
                              <strong>{recipe.batchInfo?.quantity || 1}</strong> unidad(es)
                            </span>
                          </div>
                                ...
                                <div className="mt-1 space-y-1">
                                  {recipe.ingredients
                                    ...
                                    .map((ing, idx) => (
                                      <div
                                        key={idx}
                                        className="text-xs text-green-600 ml-4"
                                      >
                                        • {ing.name || "Producto"}:{" "}
                      ...
                      {recipe.status === "completada" && (
                        <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-700">
                          <div className="flex items-center space-x-2">
                            ...
                            <span>
                              ⚠️ Cambiar el estado desde "Completada" disminuirá
                              el stock del producto producido y restaurará los
                              ingredientes consumidos
                            </span>
                          </div>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900">
                {selectedIngredientIndex === "productToProduce"
                  ? "Seleccionar Producto Terminado a Producir"
                  : "Seleccionar Materia Prima del Inventario"}
              </h3>
              <Button
                variant="ghost"
                onClick={() => {
            ...
            <div className="mb-4">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={
                    selectedIngredientIndex === "productToProduce"
                      ? "Buscar productos terminados por nombre o SKU..."
                      : "Buscar materias primas por nombre o SKU..."
                  }
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <div className="flex-1">
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4 text-xs text-gray-500 mt-1">
                            ...
                            <span>
                              Precio: {formatCurrency(product.price || 0)}
                            </span>
                          </div>
                        </div>
